...
<div class="container">
            ...
            <label>Categorie disponnible
                <select id="categorie" name="categorie" class="form-control">
                    ...
                    <c:forEach var="row" items="${requestScope.listeCategorie}">
                        <option ${(row.id == astuce)?"selected='select'":""} id="${row.id}" value="${row.id}">${row.value}</option>
                    </c:forEach>
                </select>
            </label> 
        </div>
    ...
    <c:if test="${fn:length(requestScope.listeNew) le 0}">
        <p>il n'y a pas encore de new dans cette categorie</p>
    </c:if>
    <div>
